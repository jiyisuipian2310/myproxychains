...
static void init_lib_wrapper(const char* caller) {
	...
	proxychains_write_log(LOG_PREFIX "init_lib_wrapper called from %s\n", caller);
				...
				if(strstr(buff, "[ProxyList]")) {
					...
					*ct = DYNAMIC_TYPE;
					...
					if(sscanf(local_in_addr_port, "%15[^:]:%5s", local_in_addr, local_in_port) < 2) {
						proxychains_write_log(LOG_PREFIX "added localnet: netaddr=%s, netmask=%s\n", local_in_addr, local_netmask);
					} else {
						proxychains_write_log(LOG_PREFIX "added localnet: netaddr=%s, port=%s, netmask=%s\n", local_in_addr, local_in_port, local_netmask);
					}
...
int connect(int sock, const struct sockaddr *addr, socklen_t len) {
	...
	if(!(SOCKFAMILY(*addr) == AF_INET && socktype == SOCK_STREAM)) {
		proxychains_write_log(LOG_PREFIX "11111111 begin connect\n");
	...
	proxychains_write_log(LOG_PREFIX "app want to connect to targetAddr: %s:%d\n", szIpaddr, port);
		...
		if((localnet_addr[i].in_addr.s_addr & localnet_addr[i].netmask.s_addr)
		   == (p_addr_in->s_addr & localnet_addr[i].netmask.s_addr)) {
			if(!localnet_addr[i].port || localnet_addr[i].port == port) {
				...
				proxychains_write_log(LOG_PREFIX "accessing localnet using true_connect, targetAddr: %s:%d\n", szIpaddr, port);
				return true_connect(sock, addr, len);
			}
		}
	}
	...
	proxychains_write_log(LOG_PREFIX "begin call connect_proxy_chain, proxy_count:%d\n", proxychains_proxy_count);
