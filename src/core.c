...
void proxychains_write_log(char *str, ...) {
	...
	if(!proxychains_quiet_mode) {
		if(log_file[0] != '\0') {
			...
			char timebuffer[32];
			strftime(timebuffer, sizeof(timebuffer), "%Y-%m-%d %H:%M:%S", time_info);
			sprintf(logbuff, "[%s] ", timebuffer);

			va_start(arglist, str);
			vsnprintf(logbuff+strlen(logbuff), sizeof(logbuff)-strlen(logbuff), str, arglist);
			va_end(arglist);

			FILE* fout = fopen(log_file, "a+");
			if(fout != NULL) {
				fwrite(logbuff, strlen(logbuff), 1, fout);
				fclose(fout);
			}
		} else {
			va_start(arglist, str);
			vsnprintf(logbuff, sizeof(logbuff), str, arglist);
			va_end(arglist);
			fprintf(stderr, "%s", logbuff);
		}
...
static int timed_connect(int sock, const struct sockaddr *addr, socklen_t len) {
	...
	proxychains_write_log(LOG_PREFIX "begin timed_connect\n");
...
static int start_chain_ex(int *fd, proxy_data * pd, ip_type target_ip, unsigned short target_port, char *begin_mark) {
	struct sockaddr_in addr;
	char ip_buf[16];
...
int connect_proxy_chain(int sock, ip_type target_ip,
	...
	p_addr_in = (struct in_addr *)&(target_ip.as_int);
	inet_ntop(AF_INET, p_addr_in, szIpaddr, sizeof(szIpaddr));
	//end add by yull
	...
	switch (ct) {
		case DYNAMIC_TYPE:
			proxychains_write_log(LOG_PREFIX "func connect_proxy_chain, chain_type is DYNAMIC_TYPE, targetAddr:%s:%d\n", szIpaddr, ntohs(target_port));
		...
		case STRICT_TYPE:
			proxychains_write_log(LOG_PREFIX "func connect_proxy_chain, chain_type is STRICT_TYPE, targetAddr:%s:%d\n", szIpaddr,  ntohs(target_port));
			calc_alive(pd, proxy_count);
			offset = 0;
			if(!(p1 = select_proxy(FIFOLY, pd, proxy_count, &offset))) {
		...
		case RANDOM_TYPE:
			proxychains_write_log(LOG_PREFIX "func connect_proxy_chain, chain_type is RANDOM_TYPE, targetAddr:%s:%d\n", szIpaddr,  ntohs(target_port));
